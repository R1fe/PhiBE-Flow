...
class GPEEncoder(nn.Module):
    """Encoder architecture used by the supplied Acrobot GPE T.pth weights."""

    def __init__(self, latent_dim: int = 3, image_size: int = 32):
        super().__init__()

        def block(in_channels: int, out_channels: int, use_batch_norm: bool = True):
        ...
        self.main = nn.Sequential(
            *block(1, 32),
            *block(32, 64, use_batch_norm=False),
            *block(64, 128),
        )
        self.fc_T = nn.Sequential(
            nn.Flatten(),
            nn.Linear(128 * (image_size // 8) ** 2, latent_dim),
        )

    def forward(self, images: torch.Tensor) -> torch.Tensor:
        return self.fc_T(self.main(images))


class GPEDecoder(nn.Module):
    """Decoder architecture used by the supplied Acrobot GPE S.pth weights."""
    ...
    def forward(self, latent: torch.Tensor) -> torch.Tensor:
        return self.S(latent.view(latent.shape[0], latent.shape[1], 1, 1))
