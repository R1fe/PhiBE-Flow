...
import importlib
from src.utils.serialization import load_vq_checkpoint
import math
import numpy as np
import torch
...
class VQModel(nn.Module):
    ...
    def init_from_ckpt(self, path, ignore_keys=list()):
        sd = load_vq_checkpoint(path)["state_dict"]
        keys = list(sd.keys())
        for k in keys:
        ...
        if missing:
            raise ValueError(f"Incomplete VQ-VAE checkpoint, missing keys: {missing}")
        if unexpected:
            self.info(f"Ignoring non-inference checkpoint keys: {unexpected}")
