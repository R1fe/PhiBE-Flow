...
class KTHTrainer:
    def __init__(self, model, codec, optimizer, device, config, project_root, logger):
    ...
    def load(self, path, resume=True):
        ...
        if resume and self.update_parameters:
            ...
            self.best_mse = float(checkpoint.get("best_mse", float("inf")))
    ...
    def fit(self, train_loader, test_loader, epochs, fixed_samples):
        for epoch in range(self.start_epoch+1, self.start_epoch+epochs+1):
            ...
            if self.update_parameters:
                improved = metrics["mse"] < self.best_mse
                self.best_mse = min(metrics["mse"], self.best_mse)
                extra = dict(global_step=self.global_step, ema_state_dict=self.ema.state_dict(),
                             best_mse=self.best_mse, history=self.history, config=dict(self.config),
