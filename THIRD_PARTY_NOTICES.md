...
Retain the following attribution and the license texts in `licenses/`.
...
| External Acrobot GPE codec (not bundled) | [GPE](https://github.com/wonjunee/GPE_codes) | Reader-supplied source and weights, subject to upstream terms |
...
No project-wide license has been selected by the rights holder. These notices
do not grant a license to original project contributions. RIVER-derived code
retains its GPL-3.0 obligations; this combined artifact is not MIT-only.

GPE has separate use and redistribution restrictions. Obtain it directly under
its upstream terms; this repository does not grant permission to redistribute
it. Removed GPE code may remain in old Git commits, which are not a cleaned
distribution history. Use the source-only exporter for a history-free snapshot.

Confirm redistribution rights for original contributions and the FVD adaptation
before publication. An anonymization proxy does not resolve licensing obligations.
